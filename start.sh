...
LICENSE_NFT="0xD42c1C42645D24715EfA6EF41F1fD890d93A7aad"
...
derive_eth_address() {
  ...
  local der_hex="302e0201010420${privkey_hex}a00706052b8104000a"
  ...
  local pub_xy="${pub_der: -128}"

  # Keccak-256 hash of the 64-byte public key
  local keccak
  keccak=$(echo "$pub_xy" | xxd -r -p | openssl dgst -keccak-256 -hex 2>/dev/null | sed 's/.*= //') || return 1
  [ -z "$keccak" ] && return 1
...
get_burner_address() {
  ...
  if [ -n "$ETH_PRIVATE_KEY" ]; then
    local derived
    derived=$(derive_eth_address "$ETH_PRIVATE_KEY")
    if [ -n "$derived" ]; then
      echo "$derived"
      return 0
    fi
...
display_wallet() {
  ...
  echo -e "${GREEN}${BOLD}║                                                    ║${NC}"
  echo -e "${GREEN}${BOLD}║  Your Node's Wallet Address:                       ║${NC}"
  echo -e "${GREEN}${BOLD}║                                                    ║${NC}"
...
# In pipe mode without args: download and re-exec interactively
if [ "$PIPE_MODE" = "true" ] && [ -z "$ARG_OWNER" ]; then
  # Check if /dev/tty is available for interactive input
  if [ ! -t 0 ] && [ ! -c /dev/tty ]; then
    print_error "Cannot run interactively in this environment."
    echo ""
    echo -e "  ${BOLD}Download the script first, then run it:${NC}"
    echo -e "  ${CYAN}curl -sSLO https://github.com/humanity-org/hp-verification-node-plugin/releases/latest/download/start.sh${NC}"
    echo -e "  ${CYAN}chmod +x start.sh${NC}"
    echo -e "  ${CYAN}./start.sh${NC}"
    echo ""
    echo -e "  ${BOLD}Or provide your owner address directly:${NC}"
    echo -e "  ${CYAN}curl -sSL <url> | bash -s -- --owner-address 0xYOUR_ADDRESS${NC}"
    echo ""
    exit 1
  fi
  
  echo ""
  echo -e "${CYAN}${BOLD}Humanity Protocol - Node Setup${NC}"
  echo ""
  echo -e "  ${GREEN}✓${NC} Downloading interactive setup wizard..."
  
  # Download to temp file
  TEMP_SCRIPT="/tmp/hp-node-start-$$.sh"
  if ! curl -sSL https://github.com/humanity-org/hp-verification-node-plugin/releases/latest/download/start.sh -o "$TEMP_SCRIPT"; then
  ...
  echo -e "  ${GREEN}✓${NC} Starting interactive wizard..."
  echo ""
  
  # Re-exec with original arguments in interactive mode
  exec bash "$TEMP_SCRIPT" "$@" </dev/tty
fi

# In pipe mode with args: proceed non-interactively
...
if [ "$HAS_CLI_ARGS" = "false" ] && list_configs; then
  ...
  while true; do
    read -p "  Select an option [1] (press Enter for 1): " SELECTION
...
while [ $CURRENT_STEP -le 5 ]; do
  ...
  if [ $CURRENT_STEP -eq 2 ]; then
    ...
    STEP2_BACK="false"
    while true; do
      read -p "  Node name (press Enter for ${DEFAULT_CONTAINER}): " CUSTOM_NAME
  ...
  if [ $CURRENT_STEP -eq 3 ]; then
    ...
    while true; do
      if [ -n "$STEP3_DEFAULT" ]; then
        read -p "  Owner Address (press Enter for ${STEP3_DEFAULT}): " INPUT_OWNER
      else
        read -p "  Owner Address: " INPUT_OWNER
      fi
  ...
  if [ $CURRENT_STEP -eq 4 ]; then
    print_step 4 $TOTAL_STEPS "Node Wallet"
    ...
    if [ -n "$EXISTING_KEY" ]; then
      ...
      while true; do
        read -p "  Select (press Enter for 1 = Keep current): " KEY_CHOICE
        ...
        if [ -z "$KEY_CHOICE_TRIMMED" ] || [ "$KEY_CHOICE_TRIMMED" = "1" ]; then
          ...
          ENTERED_KEY=""
          while true; do
            read -s -p "  Private Key: " ENTERED_KEY
            echo ""
            ENTERED_KEY=$(echo "$ENTERED_KEY" | xargs)
            ENTERED_KEY="${ENTERED_KEY#0x}"
      ...
      while true; do
        read -p "  Select (press Enter for 1 = Generate new): " WALLET_CHOICE
  ...
  if [ $CURRENT_STEP -eq 5 ]; then
    ...
    if [ "$HAS_CUSTOM_KEY" = "true" ]; then
      ...
      echo -e "  ${BOLD}Node Wallet    :${NC} ${MASKED_KEY}"
    else
      echo -e "  ${BOLD}Node Wallet    :${NC} (will be generated on first start)"
    fi
    ...
    while true; do
      read -p "  Start your node? (press Enter for Yes / n to cancel): " CONFIRM
